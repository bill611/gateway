...
int HAL_GetDeviceID(_OU_ char device_id[DEVICE_ID_MAXLEN])
{
    ...
    HAL_Snprintf(device_id, DEVICE_ID_MAXLEN, "%s.%s", theConfig.gate_way.product_key, theConfig.gate_way.device_name);
...
int HAL_GetDeviceName(_OU_ char device_name[DEVICE_NAME_MAXLEN])
{
    ...
    HAL_Snprintf(device_name, DEVICE_NAME_MAXLEN, "%s", theConfig.gate_way.device_name);
