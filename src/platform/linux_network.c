...
int platform_socket_device_bindto(long *handle, const char *ifname)
{
    ...
    if(NULL == ifname || strlen(ifname) == 0){
        char def_ifname[IFNAMSIZ] = {0};
        if (platform_get_default_routing_ifname(def_ifname, sizeof(def_ifname)) != NULL) {
            strncpy(itf.ifr_ifrn.ifrn_name, def_ifname, strlen(def_ifname));
        }
        else {
            perror("get default routing ifname failed");
            ret = -1;
            return ret;
        }
    }
    else
        strncpy(itf.ifr_ifrn.ifrn_name, ifname, sizeof(itf.ifr_ifrn.ifrn_name) - 1);

    if (setsockopt((long)handle, SOL_SOCKET, SO_BINDTODEVICE, (char *)&itf, sizeof(itf)) < 0) {
